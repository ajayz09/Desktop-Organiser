...
from fileExtensions import extension_paths
...
class EventHandler(FileSystemEventHandler):
    def __init__(self, watch_path: Path, organised_path: Path):
        self.watch_path = watch_path.resolve()
        print(self.watch_path)
        self.organised_path = organised_path.resolve()

    def on_modified(self, event):
        for child in self.watch_path.iterdir():
            if child.is_file() and child.suffix.lower() in extension_paths:
                destination_path = self.organised_path / extension_paths[child.suffix.lower()]
